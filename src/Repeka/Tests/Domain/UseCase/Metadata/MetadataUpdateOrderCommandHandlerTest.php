...
class MetadataUpdateOrderCommandHandlerTest extends \PHPUnit_Framework_TestCase {
    ...
    public function testUpdatingOrderOfOne() {
        ...
        $this->metadataRepository->expects($this->once())->method('findAllBase')->willReturn([$metadata]);
        ...
        $this->commandHandler->handle(new MetadataUpdateOrderCommand([1]));
    }
    ...
    public function testUpdatingOrderOfThree() {
        ...
        $this->metadataRepository->expects($this->once())->method('findAllBase')->willReturn([$metadata1, $metadata2, $metadata3]);
        ...
        $this->commandHandler->handle(new MetadataUpdateOrderCommand([2, 1, 3]));
    }
    ...
    public function testUpdatingNonExistingMetadata() {
        ...
        $this->metadataRepository->expects($this->once())->method('findAllBase')->willReturn([$metadata]);
        $metadata->expects($this->any())->method('getId')->willReturn(1);
        $this->commandHandler->handle(new MetadataUpdateOrderCommand([2]));
    }
}
