...
class FluentRestrictingMetadataSelectorTest extends \PHPUnit_Framework_TestCase {
    public function testSelectingOne() {
        $selector = new FluentRestrictingMetadataSelector([1], [2], [3]);
    ...
    public function testMergingMultiple() {
        $selector = new FluentRestrictingMetadataSelector([1, 2, 3], [3, 4], [5]);
    ...
    public function testGettingAll() {
        $selector = new FluentRestrictingMetadataSelector([1], [2], [3]);
        $this->assertEquals([1, 2, 3], $selector->all()->get());
    }

    public function testNoDuplicatesWithAll() {
        $selector = new FluentRestrictingMetadataSelector([1], [2], [3]);
        $this->assertEquals([1, 2, 3], $selector->required()->all()->get());
        $selector = new FluentRestrictingMetadataSelector([1], [2], [3]);
        $this->assertEquals([1, 2, 3], $selector->all()->required()->get());
    }
}
