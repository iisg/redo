...
class Version20170717221928 extends AbstractMigration implements ContainerAwareInterface {
    ...
    private const ADMIN_ROLE_NEW_ID = -1;
    private const OPERATOR_ROLE_NEW_ID = -2;
    ...
    private function migrateRolesUUIDtoID() {
        ...
        // migrating
        $this->addSql(
            'UPDATE role SET tmp_id = :systemUserRoleId WHERE id = :systemUserRoleUUID',
            ['systemUserRoleId' => self::ADMIN_ROLE_NEW_ID, 'systemUserRoleUUID' => self::ADMIN]
        );
        $this->addSql(
            'UPDATE role SET tmp_id = :systemUserRoleId WHERE id = :systemUserRoleUUID',
            ['systemUserRoleId' => self::OPERATOR_ROLE_NEW_ID, 'systemUserRoleUUID' => self::OPERATOR]
        );
        ...
        $this->addSql(
            'ALTER TABLE user_role ADD CONSTRAINT role_fk FOREIGN KEY(role_id) REFERENCES role(id) NOT DEFERRABLE INITIALLY IMMEDIATE'
        );
    }
    ...
    private function migrateRolesIDtoUUID() {
        ...
        // migrating
        $this->addSql(
            'UPDATE role SET tmp_id = :systemUserRoleUUID WHERE id = :systemUserRoleId',
            ['systemUserRoleId' => self::ADMIN_ROLE_NEW_ID, 'systemUserRoleUUID' => $this::ADMIN]
        );
        $this->addSql(
            'UPDATE role SET tmp_id = :systemUserRoleId WHERE id = :systemUserRoleUUID',
            ['systemUserRoleId' => self::OPERATOR_ROLE_NEW_ID, 'systemUserRoleUUID' => $this::OPERATOR]
        );
        $this->addSql('UPDATE user_role SET tmp_role_id = role.tmp_id FROM role WHERE role.id = user_role.role_id');
        // update table 'user_role'
        $this->addSql('ALTER TABLE user_role DROP role_id');
    ...
    private function updateResourceKindIds(int $prevResourceKindId) {
        ...
        $this->addSql(
            'UPDATE resource_kind SET id = :systemResourceKind WHERE id = :prevResourceKindId',
            ['systemResourceKind' => SystemResourceKind::USER, 'prevResourceKindId' => $prevResourceKindId]
        );
        $this->addSql(
            'UPDATE resource SET kind_id = :systemResourceKind WHERE kind_id = :prevResourceKindId',
            ['systemResourceKind' => SystemResourceKind::USER, 'prevResourceKindId' => $prevResourceKindId]
        );
        $this->addSql(
            'UPDATE metadata SET resource_kind_id = :systemResourceKind WHERE resource_kind_id = :prevResourceKindId',
            ['systemResourceKind' => SystemResourceKind::USER, 'prevResourceKindId' => $prevResourceKindId]
        );
        // revert foreign keys
